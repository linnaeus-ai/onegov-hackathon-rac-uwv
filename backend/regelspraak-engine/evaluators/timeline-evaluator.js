...
exports.TimelineEvaluator = void 0;
const timeline_utils_1 = require("../utils/timeline-utils");
...
class TimelineEvaluator {
     ...
     */
    evaluateTimelineBinaryOp(leftTimeline, rightTimeline, operator, context) {
        ...
        const granularity = this.getFinestGranularity(leftTimeline.granularity, rightTimeline.granularity);
        return {
            type: 'timeline',
            value: {
                periods,
    ...
    evaluateTotaal(expr, context) {
        ...
        if (targetValue.type === 'timeline') {
            const timelineValue = targetValue;
            const timeline = timelineValue.value;
            ...
            for (const period of periodsToSum) {
                ...
                total = total.plus(new decimal_js_1.default(period.value.value));
            }
            return {
                type: 'number',
    ...
    evaluateAantalDagen(expr, context) {
        ...
        if (expr.period) {
            ...
            startDate = (0, timeline_utils_1.alignDate)(context.evaluation_date, 'maand');
            endDate = (0, timeline_utils_1.nextPeriod)(startDate, 'maand');
        }
    ...
    evaluateNaarVerhouding(expr, context) {
     ...
     */
    evaluateTijdsevenredigPerMaand(expr, context) {
        const targetValue = this.expressionEvaluator.evaluate(expr.target, context);
        if (targetValue.type === 'timeline') {
            // Apply tijdsevenredig to a timeline
            return this.applyTijdsevenredigToTimeline(targetValue, 'maand', expr.condition, context);
        }
     ...
     */
    evaluateTijdsevenredigPerJaar(expr, context) {
        const targetValue = this.expressionEvaluator.evaluate(expr.target, context);
        if (targetValue.type === 'timeline') {
            // Apply tijdsevenredig to a timeline
            return this.applyTijdsevenredigToTimeline(targetValue, 'jaar', expr.condition, context);
        }
        else if (targetValue.type === 'number') {
     ...
     */
    applyTijdsevenredigToTimeline(timelineValue, periodType, condition, context) {
        const timeline = timelineValue.value;
        const resultPeriods = [];
        ...
        if (timeline.granularity !== periodType) {
            ...
            for (const period of timeline.periods) {
                ...
                const expandedPeriods = this.expandPeriodToGranularity(period, periodType, condition, context);
            ...
            for (const period of timeline.periods) {
                ...
                const periodStart = (0, timeline_utils_1.alignDate)(period.startDate, periodType);
                const periodEnd = (0, timeline_utils_1.nextPeriod)(periodStart, periodType);
                if (period.value && (period.startDate > periodStart || period.endDate < periodEnd)) {
                    // Partial period - calculate proportional value
                    const proportionalValue = (0, timeline_utils_1.calculateProportionalValue)(period.value, period.startDate, period.endDate, periodType);
                    resultPeriods.push({
     ...
     */
    expandPeriodToGranularity(period, targetGranularity, condition, context) {
        const results = [];
        ...
        if (sourceGranularity === 'jaar' && targetGranularity === 'maand') {
            ...
            const yearValue = period.value?.type === 'number' ? period.value.value : 0;
     ...
     */
    filterPeriodsWithCondition(timeline, condition, context) {
     ...
     */
    evaluatePeriodDefinition(periodDef, context) {
        let start;
        let end;
    ...
    performBinaryOp(left, right, operator) {
     ...
     */
    evaluateTimelineScalarOp(timeline, scalar, operator, context) {
        const periods = [];
        ...
        const cleanedPeriods = (0, timeline_utils_1.removeRedundantKnips)(periods, timeline.granularity);
     ...
     */
    evaluateScalarTimelineOp(scalar, timeline, operator, context) {
        const periods = [];
        // Apply the operation to each period in the timeline
        for (const period of timeline.periods) {
            if (period.value) {
...
exports.TimelineEvaluator = TimelineEvaluator;
//# sourceMappingURL=timeline-evaluator.js.map