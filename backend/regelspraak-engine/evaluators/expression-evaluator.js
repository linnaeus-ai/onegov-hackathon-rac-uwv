...
const aggregation_engine_1 = require("./aggregation-engine");
const timeline_evaluator_1 = require("./timeline-evaluator");
const predicate_evaluator_1 = require("../predicates/predicate-evaluator");
...
class ExpressionEvaluator {
    ...
    predicateEvaluator;
    constructor() {
        this.aggregationEngine = new aggregation_engine_1.AggregationEngine(this);
        this.timelineEvaluator = new timeline_evaluator_1.TimelineEvaluator(this);
        this.unitRegistry = new units_1.UnitRegistry();
        this.predicateEvaluator = new predicate_evaluator_1.PredicateEvaluator(this);
    }
    evaluate(expr, context) {
        ...
        switch (expr.type) {
            ...
            case 'TekstreeksExpression':
                return this.evaluateTekstreeksExpression(expr, context);
            case 'Literal':
            ...
            case 'VariableReference':
                return this.evaluateVariableReference(expr, context);
            case 'ParameterReference':
                return this.evaluateParameterReference(expr, context);
            case 'FunctionCall':
                return this.evaluateFunctionCall(expr, context);
            case 'AggregationExpression':
                return this.aggregationEngine.evaluate(expr, context);
            case 'TimelineExpression':
                return this.timelineEvaluator.evaluate(expr, context);
            ...
            case 'SubselectieExpression':
                return this.evaluateSubselectieExpression(expr, context);
            case 'AttributeReference':
                return this.evaluateAttributeReference(expr, context);
            case 'DimensionedAttributeReference':
                return this.evaluateDimensionedAttributeReference(expr, context);
            case 'AllAttributesExpression':
                return this.evaluateAllAttributesExpression(expr, context);
            case 'SamengesteldeVoorwaarde':
                return this.evaluateSamengesteldeVoorwaarde(expr, context);
            case 'RegelStatusExpression':
                return this.evaluateRegelStatusExpression(expr, context);
            case 'DateLiteral':
                return this.evaluateDateLiteral(expr, context);
            case 'AfrondingExpression':
                return this.evaluateAfrondingExpression(expr, context);
            case 'BegrenzingExpression':
                return this.evaluateBegrenzingExpression(expr, context);
            case 'BegrenzingAfrondingExpression':
                return this.evaluateBegrenzingAfrondingExpression(expr, context);
            case 'DisjunctionExpression':
                return this.evaluateDisjunctionExpression(expr, context);
            case 'ConjunctionExpression':
                return this.evaluateConjunctionExpression(expr, context);
            case 'VergelijkingInPredicaat':
                return this.evaluateVergelijkingInPredicaat(expr, context);
            default:
                throw new Error(`Unknown expression type: ${expr.type}`);
        }
    }
    evaluateNumberLiteral(expr, context) {
        if (expr.unit) {
            // Use context's unit registry if available (for custom unit systems)
            const registry = context.unitRegistry || this.unitRegistry;
            // Create a unit value using the unit registry
            return (0, units_1.createUnitValue)(expr.value, expr.unit, registry);
        }
        return {
            type: 'number',
            value: expr.value
    ...
    evaluateDateLiteral(expr, context) {
        return {
            type: 'date',
            value: expr.value
        };
    ...
    evaluateBinaryExpression(expr, context) {
        ...
        if (leftVal.unit || rightVal.unit || leftVal.compositeUnit || rightVal.compositeUnit) {
            try {
                // Use context's unit registry if available (for custom unit systems)
                const registry = context.unitRegistry || this.unitRegistry;
                return (0, units_1.performUnitArithmetic)(expr.operator, leftVal, rightVal, registry);
            }
            catch (error) {
                // Re-throw with more context
                throw new Error(`Unit arithmetic error: ${error.message}`);
            }
        }
        // No units - simple arithmetic
        const leftNum = leftVal.value;
        const rightNum = rightVal.value;
        let result;
    ...
    evaluateComparisonExpression(expr, left, right) {
    ...
    evaluateLogicalExpression(expr, context) {
    ...
    evaluateTimelineBinaryExpression(expr, left, right, context) {
        // Check if we can perform the operation
        const arithmeticOps = ['+', '-', '*', '/'];
        const isArithmeticOp = arithmeticOps.includes(expr.operator);
        if (left.type === 'timeline' && right.type === 'timeline') {
            // Both are timelines
            const leftTimeline = left.value;
            const rightTimeline = right.value;
            return this.timelineEvaluator.evaluateTimelineBinaryOp(leftTimeline, rightTimeline, expr.operator, context);
        }
        else if (left.type === 'timeline' && right.type === 'number' && isArithmeticOp) {
            // Timeline × scalar
            const leftTimeline = left.value;
            return this.timelineEvaluator.evaluateTimelineScalarOp(leftTimeline, right, expr.operator, context);
        }
        else if (left.type === 'number' && right.type === 'timeline' && isArithmeticOp) {
            // Scalar × timeline
            const rightTimeline = right.value;
            return this.timelineEvaluator.evaluateScalarTimelineOp(left, rightTimeline, expr.operator, context);
        }
        else {
    ...
    evaluateVariableReference(expr, context) {
        ...
        if (ctx.current_instance && ctx.current_instance.type === 'object') {
            ...
            const objectData = currentInstance.value;
    ...
    evaluateParameterReference(expr, context) {
        const ctx = context;
    ...
    aantal_special(expr, context) {
        ...
        if (arg.type === 'SubselectieExpression') {
            const subselectie = arg;
            const collection = this.resolveCollectionForAantal(subselectie.collection, context);
            ...
            for (const item of collection) {
                ...
                const previousInstance = context.current_instance;
                context.current_instance = item;
                try {
                    // Use evaluatePredicaat which properly handles KenmerkPredicaat, AttributeComparisonPredicaat
                    const matches = this.evaluatePredicaat(subselectie.predicaat, item, context);
                    if (matches) {
                        count++;
                    }
                }
                finally {
                    context.current_instance = previousInstance;
                }
            }
        ...
        if (evaluated.type === 'array') {
            return { type: 'number', value: evaluated.value.length };
        }
     ...
     */
    resolveCollectionForAantal(ref, context) {
        const path = ref.path;
        ...
        if (path.length === 1) {
            ...
            const variable = context.getVariable(typeName);
            ...
            if (context.current_instance) {
                const related = context.getRelatedObjects(context.current_instance, typeName, true // as subject
                );
        ...
        if (path.length === 2) {
            ...
            if (context.current_instance) {
                const currentAttrs = context.current_instance.value || {};
            ...
            if (!contextObj) {
                const objects = context.getObjectsByType(contextRef);
            ...
            if (contextObj) {
                const related = context.getRelatedObjects(contextObj, roleName, true);
                if (related && related.length > 0) {
                    return related;
                }
                // Also try as object (in relationship)
                const relatedAsObject = context.getRelatedObjects(contextObj, roleName, false);
    ...
    som_van_special(expr, context) {
        ...
        if (attrArg.type === 'AttributeReference') {
            attrName = attrArg.path[attrArg.path.length - 1];
        }
        else if (attrArg.type === 'VariableReference') {
            attrName = attrArg.variableName;
        }
        else {
     ...
     * arg[1] = collection path to iterate
     */
    aggregation_special(expr, context, reducer, initialValue) {
        const attrArg = expr.arguments[0];
        const collectionArg = expr.arguments[1];
        // First evaluate the collection argument
        const collectionValue = this.evaluate(collectionArg, context);
        ...
        for (const item of items) {
            if (item.type === 'object') {
                ...
                if (attrValue !== undefined && attrValue.type === 'number') {
                    result = reducer(result, attrValue.value);
                }
            }
        }
    ...
    minimum_van(args) {
        ...
        for (const val of values) {
            if (val.type === 'number') {
                minValue = Math.min(minValue, val.value);
            }
        }
        return {
            type: 'number',
    ...
    tijdsduur_van(args, unitConversion) {
        if (args.length !== 2) {
        ...
        const startDate = startDateVal.value;
        const endDate = endDateVal.value;
    ...
    abs_tijdsduur_van(args, unitConversion) {
        ...
        if (result.type === 'number') {
            ...
            if ('unit' in result) {
                return (0, units_1.createUnitValue)(value, result.unit?.name);
            }
            return {
                type: 'number',
    ...
    aantal_dagen_in_special(expr, context) {
        ...
        while (currentDate <= endDate) {
            ...
            Object.assign(tempContext, {
                current_instance: context.current_instance,
                getEvaluationDate: function () { return new Date(currentDate); },
                setEvaluationDate: function (date) { },
                ...
                getObjectsByType: context.getObjectsByType ? context.getObjectsByType.bind(context) : undefined
            });
    ...
    findRelatedObjectsThroughFeittype(roleName, fromObject, context) {
        ...
        for (const feittype of feittypen) {
            ...
            for (let roleIdx = 0; roleIdx < feittype.rollen.length; roleIdx++) {
                ...
                if (roleNameClean === rolNaamClean ||
                    ...
                    const fromObjType = fromObject.objectType;
                    if (!fromObjType)
                        continue;
    ...
    evaluateAttributeReference(expr, context) {
        ...
        if (expr.path.length > 1) {
            ...
            if (lastElement === 'aantal' || lastElement === 'som' || lastElement === 'totaal') {
                ...
                if (lastElement === 'aantal') {
                    // Count the items
                    if (collectionValue.type === 'array') {
                        const items = collectionValue.value;
        ...
        if (expr.path.length === 2 && expr.path[0] === 'self') {
            ...
            if (ctxAny.getTimelineAttribute && currentInstance.objectType && currentInstance.objectId) {
                ...
                if (ctxAny.getTimelineAttributeNames) {
                    ...
                    const timelineAttrNames = ctxAny.getTimelineAttributeNames(currentInstance.objectType, currentInstance.objectId) || [];
                ...
                for (const nameToTry of namesToTry) {
                    const timelineValue = ctxAny.getTimelineAttribute(currentInstance.objectType, currentInstance.objectId, nameToTry, ctxAny.evaluation_date);
            ...
            const objectData = currentInstance.value;
            let val = objectData[attrName] || objectData[attr];
        ...
        if (expr.path.length === 3 && expr.path[1] === 'alle') {
            ...
            const ctx = context; // Cast to access implementation-specific methods
        ...
        if (expr.path.length === 2) {
            ...
            if (ctx.current_instance && ctx.current_instance.type === 'object') {
                ...
                if (currentObjType && objectName.toLowerCase() === currentObjType.toLowerCase()) {
                    ...
                    const instanceData = ctx.current_instance.value;
                ...
                if (ctx.current_instance && ctx.current_instance.type === 'object') {
                    ...
                    if (directAttr !== undefined && directAttr !== null) {
                        ...
                        if (directAttr.type === 'object') {
                            const nestedData = directAttr.value;
                            ...
                            if (objValue && typeof objValue === 'object') {
                                const attrValue = objValue[attribute];
                                if (attrValue !== undefined) {
                                    return attrValue;
                                }
                            }
                    ...
                    if (relatedObjects && relatedObjects.length > 0) {
                        ...
                        if (relatedObject.type === 'object') {
                            const objectData = relatedObject.value;
            ...
            const objectData = navResult.targetObject.value;
            ...
            if (value === undefined) {
                const { name: strippedName, unit } = (0, navigation_1.stripUnitSuffix)(navResult.attributeName);
            ...
            if (value === undefined) {
                const relatedObjects = this.findRelatedObjectsThroughFeittype(effectiveAttrName, navResult.targetObject, context);
                if (relatedObjects && relatedObjects.length > 0) {
        ...
        if (expr.path.length === 1) {
            ...
            if (ctx.current_instance) {
                ...
                if (currentInstance.type === 'object' && currentInstance.value) {
                    const instanceData = currentInstance.value;
    ...
    evaluateSubselectieExpression(expr, context) {
        ...
        if (collection.type === 'AttributeReference' && collection.path && collection.path.length > 1) {
            ...
            const collectionValue = this.evaluate(collectionRef, context);
    ...
    evaluatePredicaat(predicaat, item, context) {
        switch (predicaat.type) {
            case 'KenmerkPredicaat':
                return this.evaluateKenmerkPredicaat(predicaat, item, context);
            case 'AttributeComparisonPredicaat':
                return this.evaluateAttributeComparisonPredicaat(predicaat, item, context);
            default:
                throw new Error(`Unknown predicaat type: ${predicaat.type}`);
        }
    }
    evaluateKenmerkPredicaat(predicaat, item, context) {
        if (item.type !== 'object') {
            return false;
        }
        const itemAny = item;
        ...
        const kenmerken = itemAny.kenmerken;
        if (kenmerken) {
            // Try various forms
        ...
        if (kenmerkValue && kenmerkValue.type === 'boolean') {
            return kenmerkValue.value;
        }
        return false;
    }
    evaluateAttributeComparisonPredicaat(predicaat, item, context) {
        ...
        const comparisonExpr = {
            ...
            operator: predicaat.operator,
            left: { type: 'VariableReference', variableName: '_temp' },
            right: predicaat.value
        };
        // Create temporary context with the attribute value
        const tempContext = context;
        tempContext.setVariable('_temp', attributeValue);
        const result = this.evaluateBinaryExpression(comparisonExpr, tempContext);
        // Clean up temporary variable
        tempContext.setVariable('_temp', undefined);
    ...
    evaluateDagsoortExpression(expr, context) {
        ...
        const dagsoortName = dagsoortExpr.value;
        const date = dateValue.value;
        // Check if the dagsoort is declared in the model
        // Need to check both the full name and the name without article
        const isDagsoortDeclared = context.domainModel.dagsoortDefinities?.some(d => {
            const fullName = d.name || d.dagsoortName;
            if (!fullName)
                return false;
            // Check exact match
            if (fullName.toLowerCase() === dagsoortName.toLowerCase())
                return true;
        ...
        const dagsoortRules = (context.domainModel.regels || []).filter(regel => {
            ...
            if (result && result.type === 'DagsoortDefinitie' &&
                result.dagsoortName?.toLowerCase() === dagsoortName.toLowerCase()) {
                return true;
            }
        ...
        const dagObject = {
            ...
            attributes: {
                dag: { type: 'date', value: date },
                maand: { type: 'number', value: date.getMonth() + 1 },
                dag_van_maand: { type: 'number', value: date.getDate() },
                jaar: { type: 'number', value: date.getFullYear() }
            }
        };
        // Set the dag object as current instance
        dagContext.current_instance = dagObject;
        ...
        for (const regel of dagsoortRules) {
            ...
            if (voorwaarde) {
                try {
                    ...
                    const conditionResult = this.evaluate(voorwaarde.expression || voorwaarde.expressie, dagContext);
    ...
    evaluateNumericExactExpression(expr, context) {
        ...
        const digitCount = digitCountExpr.value;
     ...
     */
    evaluateTekstreeksExpression(expr, context) {
        let result = '';
     ...
     */
    formatValueForInterpolation(value) {
    ...
    evaluateUniekExpression(operand, context) {
        ...
        const values = collectionValue.value;
    ...
    evaluateAllAttributesExpression(expr, context) {
    ...
    evaluateRegelStatusExpression(expr, context) {
        // Evaluate rule status check (gevuurd/inconsistent)
        const ctx = context; // Cast to access Context methods
    ...
    evaluateSamengesteldeVoorwaarde(voorwaarde, context) {
        ...
        if (voorwaarde.voorwaarden && voorwaarde.voorwaarden.length > 0) {
            ...
            if (kwantType === 'alle' || kwantType === expressions_1.QuantifierType.ALLE) {
                // All conditions must be true
                finalResult = conditionsMetCount === totalConditions;
            }
            else if (kwantType === 'geen' || kwantType === expressions_1.QuantifierType.GEEN) {
                // No conditions can be true
                finalResult = conditionsMetCount === 0;
            }
            else if (kwantType === 'ten_minste' || kwantType === expressions_1.QuantifierType.TEN_MINSTE) {
                // At least n conditions must be true
                const aantal = voorwaarde.kwantificatie?.aantal ?? 1;
                finalResult = conditionsMetCount >= aantal;
            }
            else if (kwantType === 'ten_hoogste' || kwantType === expressions_1.QuantifierType.TEN_HOOGSTE) {
                // At most n conditions must be true
                const aantal = voorwaarde.kwantificatie?.aantal ?? 1;
                finalResult = conditionsMetCount <= aantal;
            }
            else if (kwantType === 'precies' || kwantType === expressions_1.QuantifierType.PRECIES) {
        ...
        if (voorwaarde.predicate) {
            ...
            const result = this.predicateEvaluator.evaluate(voorwaarde.predicate, { type: 'null', value: null }, context);
    ...
    evaluateDimensionedAttributeReference(expr, context) {
        ...
        if (baseAttribute.type === 'AttributeReference') {
            ...
            const subselectieResult = this.evaluateSubselectieExpression(baseAttribute, context);
            // For dimensional access, we need a single object, not a collection
            if (subselectieResult.type === 'array') {
                const items = subselectieResult.value;
                if (items.length === 0) {
                    return { type: 'null', value: null };
                }
        ...
        const objectData = targetObject.value;
     ...
     */
    evaluateBuiltInDagsoort(date, dagsoortName) {
    ...
    isDutchPublicHoliday(date) {
        ...
        if (month === 3) {
            if (day === 27 && date.getDay() !== 0)
                return true;
            if (day === 26 && new Date(year, 3, 27).getDay() === 0)
                return true;
        }
        // Liberation Day (May 5) - every 5 years
        if (month === 4 && day === 5 && year % 5 === 0)
            return true;
    ...
    eerste_paasdag_van(args) {
        ...
        return {
            type: 'date',
            value: this.calculateEasterSunday(yearArg.value)
        };
    }
     ...
     */
    evaluateBegrenzingExpression(expr, context) {
        const innerValue = this.evaluate(expr.expression, context);
        const numValue = this.toNumber(innerValue);
        if (numValue === null)
            return innerValue;
     ...
     */
    evaluateBegrenzingAfrondingExpression(expr, context) {
     ...
     */
    evaluateDisjunctionExpression(expr, context) {
        ...
        if (expr.location && context.sourceText) {
            ...
            if (line) {
                ...
                if (stringMatches && stringMatches.length > 0) {
                    const values = stringMatches.map(s => ({
                        type: 'string',
                        value: s.replace(/'/g, '')
                    }));
     ...
     */
    evaluateConjunctionExpression(expr, context) {
        if (expr.values && expr.values.length > 0) {
            const evaluatedValues = [];
            for (const val of expr.values) {
                evaluatedValues.push(this.evaluate(val, context));
     ...
     */
    evaluateVergelijkingInPredicaat(expr, context) {
        switch (expr.vergelijkingType) {
            case 'kenmerk_check': {
                ...
                if (subjectValue.type === 'object') {
                    ...
                    const objectType = subjectValue.objectType || '';
                    const objectId = subjectValue.objectId || '';
                    // Use Context's getKenmerk method which handles normalization
                    const hasKenmerk = context.getKenmerk(objectType, objectId, kenmerkNaam);
            ...
            default:
                throw new Error(`Unknown vergelijkingType: ${expr.vergelijkingType}`);
        }
    }
}
