...
class CompositeUnit {
    ...
    toString() {
        ...
        const numParts = this.numerator.map(([unit, power]) => power === 1 ? unit : `${unit}^${power}`);
        if (this.denominator.length === 0) {
            return numParts.join("*");
        }
        const denParts = this.denominator.map(([unit, power]) => power === 1 ? unit : `${unit}^${power}`);
 ...
 */
function parseUnitWithPower(unitStr) {
...
//# sourceMappingURL=composite-unit.js.map