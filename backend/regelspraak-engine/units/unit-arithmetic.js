...
exports.performUnitArithmetic = performUnitArithmetic;
const composite_unit_1 = require("./composite-unit");
/**
 * Create a value with unit information
 */
function createUnitValue(value, unit, registry) {
    if (!unit) {
        return { type: 'number', value };
    }
    // Parse composite unit
    const compositeUnit = composite_unit_1.CompositeUnit.parse(unit);
 ...
 */
function performUnitArithmetic(operation, left, right, registry) {
    switch (operation) {
        case '+':
        case '-':
...
function performAdditionSubtraction(operation, left, right, registry) {
...
function performMultiplication(left, right) {
...
function performDivision(left, right) {
    if (right.value === 0) {
    ...
    const leftUnit = leftComposite || new composite_unit_1.CompositeUnit();
    const rightUnit = rightComposite || new composite_unit_1.CompositeUnit();
    // Divide units
 ...
 */
function getCompositeUnit(value) {
    if (value.compositeUnit) {
        return value.compositeUnit;
    }
    if (value.unit) {
        // Create composite unit from simple unit
        return new composite_unit_1.CompositeUnit([[value.unit.name, 1]], []);
    }
    return undefined;
}
/**
...
//# sourceMappingURL=unit-arithmetic.js.map