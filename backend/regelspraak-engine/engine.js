...
const rule_executor_1 = require("./executors/rule-executor");
const decision_table_executor_1 = require("./executors/decision-table-executor");
const parser_1 = require("./parser");
const unit_registry_1 = require("./units/unit-registry");
const base_unit_1 = require("./units/base-unit");
/**
 * Main RegelSpraak engine
 */
class Engine {
    expressionEvaluator = new expression_evaluator_1.ExpressionEvaluator();
    ruleExecutor = new rule_executor_1.RuleExecutor();
    decisionTableExecutor = new decision_table_executor_1.DecisionTableExecutor();
    antlrParser = new parser_1.AntlrParser();
    unitRegistry = new unit_registry_1.UnitRegistry();
    parse(source) {
        ...
        try {
            ...
            if (hasMultipleDefinitions) {
                ...
                const rules = definitions.filter((def) => def.type === 'Rule');
                const objectTypes = definitions.filter((def) => def.type === 'ObjectTypeDefinition');
                const parameters = definitions.filter((def) => def.type === 'ParameterDefinition');
                const unitSystems = definitions.filter((def) => def.type === 'UnitSystemDefinition');
                const dimensions = definitions.filter((def) => def.type === 'Dimension');
                const feittypen = definitions.filter((def) => def.type === 'FeitType');
                const regelGroepen = definitions.filter((def) => def.type === 'RegelGroep');
                return {
                    success: true,
                    ast: {
                        type: 'Model',
        ...
        try {
            ...
            if (!ast.type && (ast.regels || ast.objectTypes || ast.parameters)) {
                ...
                for (const beslistabel of beslistabels) {
                    ...
                    if (!targetType)
                        continue;
                    const instances = context.getObjectsByType(targetType);
                ...
                for (const rule of (ast.regels || [])) {
                    ...
                    for (const instance of instances) {
                        ...
                        try {
                            ...
                            const previousIterationFlag = context._engineControlsIteration;
                            context._engineControlsIteration = true;
                            try {
                                const result = this.ruleExecutor.execute(rule, context);
                                if (!result.success) {
                                    return { success: false, error: result.error };
                                ...
                                context._engineControlsIteration = previousIterationFlag;
                            }
                        }
                        catch (e) {
                ...
                // Register all FeitTypes before executing rules
                for (const feittype of ast.feittypen || []) {
                    if (context.registerFeittype) {
                        context.registerFeittype(feittype);
                    }
                }
                ...
                const beslistabels = ast.beslistabels || ast.decisionTables || [];
                // ============================================================
                // Phase 1: Execute decision tables that provide lookup values
    ...
    run(source, context) {
        ...
        if (context) {
            ...
            if (ast && (ast.type === 'Model' || (!ast.type && (ast.dimensions || ast.objectTypes)))) {
                ...
                if (ctx.initializeKenmerkRegistries) {
                    ctx.initializeKenmerkRegistries();
                }
            }
        }
        else {
            ...
            if (ast && (ast.type === 'Model' || (!ast.type && (ast.dimensions || ast.objectTypes)))) {
                const model = ast.type === 'Model' ? ast : ast;
                ctx = new context_1.Context(model);
            }
            else {
    ...
    registerUnitSystem(unitSystemDef, context) {
        // Create a new unit system
        const system = new base_unit_1.UnitSystem(unitSystemDef.name);
        ...
        const computeToBaseFactor = (unitName, visited) => {
        ...
        context.unitRegistry = this.unitRegistry;
    }
    /**
     ...
     */
    deduceBeslistabelTargetType(table, context) {
        let rawType;
     ...
     */
    deduceObjectCreationSourceType(rule, context) {
        ...
        if (!rule.condition && !rule.voorwaarde) {
            ...
            const feittypen = context.getAllFeittypen?.() || [];
        ...
        const extractTypeReferences = (expr) => {
            if (!expr)
                return;
            ...
            if (expr.left)
                extractTypeReferences(expr.left);
            if (expr.right)
                extractTypeReferences(expr.right);
            if (expr.operand)
                extractTypeReferences(expr.operand);
            if (expr.arguments) {
            ...
            if (expr.expression)
                extractTypeReferences(expr.expression);
            if (expr.collection)
                extractTypeReferences(expr.collection);
        };
        ...
        for (const candidate of candidates) {
            const instances = context.getObjectsByType(candidate);
            ...
            for (const variant of variations) {
                const varInstances = context.getObjectsByType(variant);
     ...
     */
    extractAttributeReferences(expr) {
        const refs = [];
        if (!expr)
            return refs;
     ...
     */
    roleAliasToObjectType(name, context) {
        if (!name)
            return null;
        const ctx = context;
     ...
     */
    deduceRuleTargetType(rule, context) {
     ...
     */
    deduceTypeFromAttributeTarget(resultaat, rule, context) {
        const ctx = context;
     ...
     */
    deduceTypeForObjectCreation(resultaat, rule, context) {
     ...
     */
    deduceTypeForConsistentieregel(resultaat, rule, context) {
        const ctx = context;
     ...
     */
    deduceTypeForVerdeling(resultaat, context) {
            ...
            for (const pathElem of resultaat.sourceAmount.path) {
                ...
                if (ctx.domainModel?.objectTypes) {
                    for (const objType of ctx.domainModel.objectTypes) {
                        ...
                        if (objTypeWords.every((word) => words.includes(word))) {
                            return objType.name;
                        }
                    }
                }
     ...
     */
    deduceTypeForFeitCreatie(resultaat, context) {
        // Try subject1 first
     ...
     */
    deduceTypeFromSubjectRef(subjectRef, context) {
        if (!subjectRef || !subjectRef.path)
            return null;
        const ctx = context;
