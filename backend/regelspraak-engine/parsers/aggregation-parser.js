"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AggregationParser = void 0;
/**
 * Parser for RegelSpraak aggregation expressions
 */
class AggregationParser {
    input;
    position = 0;
    constructor(input) {
        this.input = input;
    }
    /**
     * Parse aggregation expressions like:
     * - "de som van X, Y en Z"
     * - "het aantal personen"
     * - "de maximale waarde van alle bedragen"
     */
    parseAggregation() {
        this.skipWhitespace();
        // Try to match aggregation patterns
        const aggregationType = this.parseAggregationType();
        if (!aggregationType) {
            return null;
        }
        // Skip "van" if present
        this.skipWhitespace();
        if (this.matchWord('van')) {
            this.skipWhitespace();
        }
        // Parse target expressions
        const target = this.parseTarget();
        return {
            type: 'AggregationExpression',
            aggregationType,
            target
        };
    }
    parseAggregationType() {
        const patterns = [
            { pattern: 'de som', type: 'som' },
            { pattern: 'het aantal', type: 'aantal' },
            { pattern: 'de maximale waarde', type: 'maximum' },
            { pattern: 'de minimale waarde', type: 'minimum' },
            { pattern: 'de eerste', type: 'eerste' },
            { pattern: 'de laatste', type: 'laatste' }
        ];
        for (const { pattern, type } of patterns) {
            if (this.match(pattern)) {
                return type;
            }
        }
        return null;
    }
    parseTarget() {
        const expressions = [];
        // Parse first expression
        const expr = this.parseSimpleExpression();
        if (expr) {
            expressions.push(expr);
        }
        // Check for comma-separated list
        while (this.position < this.input.length) {
            this.skipWhitespace();
            if (this.matchChar(',')) {
                this.skipWhitespace();
                const nextExpr = this.parseSimpleExpression();
                if (nextExpr) {
                    expressions.push(nextExpr);
                }
            }
            else if (this.matchWord('en')) {
                this.skipWhitespace();
                const lastExpr = this.parseSimpleExpression();
                if (lastExpr) {
                    expressions.push(lastExpr);
                }
                break;
            }
            else {
                break;
            }
        }
        return expressions.length === 1 ? expressions[0] : expressions;
    }
    parseSimpleExpression() {
        // For now, just parse variable references
        // In a full implementation, this would parse more complex expressions
        const start = this.position;
        while (this.position < this.input.length &&
            this.isLetter(this.input[this.position])) {
            this.position++;
        }
        if (this.position === start) {
            return null;
        }
        const name = this.input.substring(start, this.position);
        return {
            type: 'VariableReference',
            variableName: name
        };
    }
    match(pattern) {
        const remaining = this.input.substring(this.position);
        if (remaining.toLowerCase().startsWith(pattern.toLowerCase())) {
            this.position += pattern.length;
            return true;
        }
        return false;
    }
    matchWord(word) {
        const remaining = this.input.substring(this.position);
        const pattern = new RegExp(`^${word}\\b`, 'i');
        const match = remaining.match(pattern);
        if (match) {
            this.position += match[0].length;
            return true;
        }
        return false;
    }
    matchChar(char) {
        if (this.input[this.position] === char) {
            this.position++;
            return true;
        }
        return false;
    }
    skipWhitespace() {
        while (this.position < this.input.length &&
            /\s/.test(this.input[this.position])) {
            this.position++;
        }
    }
    isLetter(ch) {
        return /[a-zA-Z]/.test(ch);
    }
}
exports.AggregationParser = AggregationParser;
//# sourceMappingURL=aggregation-parser.js.map