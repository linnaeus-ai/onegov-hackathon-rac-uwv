...
const originalProfilesPath = resolve(
  __dirname,
  "../rules-as-code-pension-starter/data/Hackathon_profiles.yaml",
);
const profilesPath = existsSync(localProfilesPath)
  ? localProfilesPath
  : originalProfilesPath;
let profiles = {};
...
function transformProfile(profileId, profile) {
  ...
  const pensioenvermogenOP =
    (pfzw["Pensioenvermogen OP"] || 0) + (abp["Pensioenvermogen OP"] || 0);
  const opVerwachtMaand =
    (pfzw["OP_verwacht_maand"] || 0) + (abp["OP_verwacht_maand"] || 0);
  ...
  if (isAOWGerechtigd) {
    if (hasPartner) {
      aowInkomen = aowBrutoAlleenstaand * 0.5 * (aowPercentage / 100); // 50% for partners
    } else {
      aowInkomen = aowBrutoAlleenstaand * 0.7 * (aowPercentage / 100); // 70% for single
    }
  }
...
// Get all profiles (limited to 10, only those with pension data and age 67)
  ...
  const profileList = Object.entries(profiles)
    .map(([id, profile]) => {
      ...
      return {
        ...
        age: transformed.age,
        hasPensionData:
          transformed.pensioenvermogen > 0 ||
          transformed.aanvullendPensioenPerMaand > 0,
      };
    })
    // Filter: only age 67 profiles
    .filter((p) => p.age >= 67)
...
app.post("/api/calculate", (req, res) => {
  ...
  if (profileId && profiles[profileId]) {
    ...
    return res
      .status(400)
      .json({ error: "Profile ID or profile data required" });
  }
  ...
  const percentage =
    opnamePercentage !== undefined && opnamePercentage !== null
      ? opnamePercentage
      : 10;
  ...
  res.json({
    ...
    baseline,
    comparison: {
      inkomenVerschil: result.nettoInkomen - baseline.nettoInkomen,
      belastingVerschil:
        result.belastingNaHeffingskortingen -
        baseline.belastingNaHeffingskortingen,
...
app.post("/api/calculate-optimal", (req, res) => {
  ...
  for (let pct = 0; pct <= 10; pct++) {
    ...
    const yearOneIncome =
      result.nettoInkomen + result.zorgtoeslag + result.huurtoeslag;
    ...
    const subsequentYearResult = calculateScenario(
      {
        ...
        aanvullendPensioenPerMaand:
          (persoon.aanvullendPensioenPerMaand * (100 - pct)) / 100,
      },
      0,
    );

    const subsequentYearIncome =
      subsequentYearResult.nettoInkomen +
    ...
    const lifetimeEquity =
      yearOneIncome + subsequentYearIncome * (remainingYears - 1);
