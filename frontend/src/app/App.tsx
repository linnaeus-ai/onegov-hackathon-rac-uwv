...
export default function App() {
  ...
  const results = useMemo(() => {
    ...
    const ongoingMonthly = Math.round(
      baseline.maandelijksBeschikbaarInkomen - result.maandelijksVerlies,
    );
    ...
    return {
      ...
      incomeChartData: [
        {
          name: "Zonder opname",
          waarde: Math.round(baseline.maandelijksBeschikbaarInkomen),
        },
        {
          name: "Jaar van opname",
          waarde: Math.round(result.maandelijksBeschikbaarInkomen),
        },
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <ImpactCard
                      type="toeslagen"
                      title="Toeslagen"
                      description="Effect op zorgtoeslag en huurtoeslag in het jaar van de eenmalige opname."
                      value={(() => {
                        const zorgtoeslagVerlies = Math.round(
                          calculation.baseline.zorgtoeslag -
                            calculation.result.zorgtoeslag,
                        );
                        const huurtoeslagVerlies = Math.round(
                          calculation.baseline.huurtoeslag -
                            calculation.result.huurtoeslag,
                        );
                        const totaalVerlies =
                          zorgtoeslagVerlies + huurtoeslagVerlies;
                        return totaalVerlies > 0
                          ? `- €${totaalVerlies.toLocaleString()}`
                          : "Geen verlies";
                      })()}
                      trend={(() => {
                        const zorgtoeslagVerlies = Math.round(
                        ...
                        const totaalVerlies =
                          zorgtoeslagVerlies + huurtoeslagVerlies;
                        return totaalVerlies > 0 ? "Verlies" : "Neutraal";
                      })()}
                      onClick={() => setActiveModal("toeslagen")}
                    />
                  </div>
          ...
          activeModal === "income"
            ...
            : activeModal === "tax"
              ? {
                  ...
                  // Phase 3 details (tax brackets and Zvw)
                  zvwBijdrage: calculation?.result.zvwBijdrage || 0,
                  belastingEersteSchijf:
                    calculation?.result.belastingEersteSchijf || 0,
                  belastingTweedeSchijf:
                    calculation?.result.belastingTweedeSchijf || 0,
                  belastingDerdeSchijf:
                    calculation?.result.belastingDerdeSchijf || 0,
                  // Phase 4 details (individual heffingskortingen)
                  algemeneHeffingskorting:
                    calculation?.result.algemeneHeffingskorting || 0,
                  ouderenkorting: calculation?.result.ouderenkorting || 0,
                  alleenstaandeOuderenkorting:
                    calculation?.result.alleenstaandeOuderenkorting || 0,
                  baselineAlgemeneHeffingskorting:
                    calculation?.baseline.algemeneHeffingskorting || 0,
                  baselineOuderenkorting:
                    calculation?.baseline.ouderenkorting || 0,
                  baselineAlleenstaandeOuderenkorting:
                    calculation?.baseline.alleenstaandeOuderenkorting || 0,
                  // Profile data
                  isAlleenstaand:
                    calculation?.profile.burgerlijkeStaat === "alleenstaand",
                  // Parameters (2024 tax rules)
                  parameters: {
                    eersteSchijfgrens: 40021,
                    tweedeSchijfgrens: 75518,
                    tariefEersteSchijfAOW: 0.1907,
                    tariefTweedeSchijf: 0.3693,
                    tariefDerdeSchijf: 0.495,
                    zvwPercentage: 0.0545,
                    zvwMaximum: 73031,
                    ahkMaximumAOW: 1735,
                    ahkAfbouwgrens: 24813,
                    ahkAfbouwPercentageAOW: 0.03421,
                    okMaximum: 1835,
                    okAfbouwgrens: 44770,
                    okAfbouwPercentage: 0.15,
                    aokBedrag: 532,
                    ztGrensAlleenstaand: 37496,
                    ztMaximumAlleenstaand: 1800,
                    ztDrempel: 25437,
                    ztAfbouwPercentage: 0.1322,
                    htGrensEerste: 26000,
                    htGrensTweede: 32000,
                    htGrensDerde: 38000,
                    htMaximum: 2900,
              ...
              : {
                  ...
                  huurtoeslag: calculation?.result.huurtoeslag || 0,
                  baselineHuurtoeslag: calculation?.baseline.huurtoeslag || 0,
                  // Parameters (for threshold calculations)
                  parameters: {
                    ztGrensAlleenstaand: 37496,
                    ztMaximumAlleenstaand: 1800,
                    ztDrempel: 25437,
